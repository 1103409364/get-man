...
async function addHistoryEntry(entry) {
  ...
  if (count >= MAX_HISTORY) {
    const oldest = await db.history.orderBy('timestamp').limit(count - MAX_HISTORY + 1).toArray()
...
async function saveRequest(request, collectionId) {
  const siblings = await db.requests.where('collectionId').equals(collectionId).toArray()
  const maxOrder = siblings.length > 0 ? Math.max(...siblings.map(s => s.order || 0)) : -1
